...
- **김광희** : 이번 자율 프로젝트에서 모든 팀원들은 이전 까지 해왔던 프로젝트 경험으로 각자 맡은 역할에 책임감을 가지는건 물론 프로젝트에 열심히 임해 주어서 프로젝트가 잘 마무리 된 것 같습니다. 또한 Git Flow, Jira, Notion, Discord를 사용하여 비대면 상황이더라도 서로의 부족한 점을 효율적으로 소통하고 협업을 할 수 있었습니다. 이번에도 ("Local에선 잘 실행 되는데 Server 배포한 곳에선 왜 안 되지?") 에러를 Jenkins를 하며 마주치게 되어 많이 힘들었지만, 코치님과 컨설턴트님이 도와주실려고 노력을 해 주셔서 잘 해결할 수 있었습니다. 마지막으로 우리 "진정해줘연" 팀원들 모두 수고했고 얼마 남지 않은 SSAFY 기간 다 같이 취뽀 화이팅!!!😀😀
