...
/**
 *	카카오 Auth 및 카카오 유저 관련 비즈니스 로직 처리를 위한 서비스 구현 정의.
 */
...
public class KaKaoServiceImpl implements KaKaoService{
    ...
    public String getAccessToken(String authorize_code) {
        ...
        try {
            ...
            sb.append("&redirect_uri=https://k6e103.p.ssafy.io:8443/oauth/kakao/callback");
...
//            JsonObject profile = element.getAsJsonObject().get("profile").getAsJsonObject();
            ...
            String profile_url = profile.getAsJsonObject().get("thumbnail_image_url").getAsString();
            String email = kakao_account.getAsJsonObject().get("email").getAsString();

            userInfo.put("email", email);
            userInfo.put("nickname", nickname);
            userInfo.put("profile", profile_url);
    ...
    @Override
    public User registerUser(KaKaoUserRegisterReq kaKaoUserRegisterReq) {
        System.out.println("debug >>>>>>>>>>>>> " + kaKaoUserRegisterReq.toString());
        User user = User.builder()
                        .userEmail(kaKaoUserRegisterReq.getUserEmail())
                        .userNickname(kaKaoUserRegisterReq.getUserNickname())
